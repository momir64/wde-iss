...
public interface ListingRepository extends JpaRepository<VersionedProduct, VersionedProductId> {
    @Query(value = """
                        ...
                        from (
                            ...
                            from (
                                (select 'SERVICE' as type, vs.static_service_id as id, version, vs.name, vs.description, price as oldPrice, (price * (1 - sale_percentage)) as price,
                                        is_available, is_active, is_private, city, images, vs.is_last_version,
                                ...
                                (select 'PRODUCT' as type, vp.static_product_id as id, version, vp.name, vp.description, price as oldPrice, (price * (1 - sale_percentage)) as price,
                                        is_available, is_active, is_private, city, images, vp.is_last_version,
    ...
    @Query(value = """
                        with ranked as (
                            select *, dense_rank() over (order by
                                                            case when :sortBy = 'price' and :order = 'asc' then price end,
                                                            case when :sortBy = 'price' and :order = 'desc' then price end desc,
                                                            case when :sortBy = 'name' and :order = 'asc' then name end,
                                                            case when :sortBy = 'name' and :order = 'desc' then name end desc,
                                                            case when :sortBy = 'rating' and :order = 'asc' then rating end,
                                                            case when :sortBy = 'rating' and :order = 'desc' then rating end desc,
                                                         id) as rank
                            from (
                                (select 'SERVICE' as type, vs.static_service_id as id, version, vs.name, vs.description, price as oldPrice, (price * (1 - sale_percentage)) as price,
                                        is_available, is_active, is_private, images, ss.service_category_id as category, vs.is_last_version,
                                        coalesce((select avg(lr.grade) from listing_review lr where lr.service_static_service_id = ss.static_service_id and lr.pending_status = 'APPROVED'), 0.0) as rating
                                from versioned_service vs
                                inner join static_service ss on ss.static_service_id = vs.static_service_id
                                inner join seller s on s.id = ss.seller_id
                                ...
                                (select 'PRODUCT' as type, vp.static_product_id as id, version, vp.name, vp.description, price as oldPrice, (price * (1 - sale_percentage)) as price,
                                        is_available, is_active, is_private, images, sp.product_category_id as category, vp.is_last_version,
                                        coalesce((select avg(lr.grade) from listing_review lr where lr.product_static_product_id = sp.static_product_id and lr.pending_status = 'APPROVED'), 0.0) as rating
                                from versioned_product vp
                                inner join static_product sp on sp.static_product_id = vp.static_product_id
                                inner join seller s on s.id = sp.seller_id
    ...
    @Query(value = """
                        with ranked_from_seller as (
                            ...
                            from (
                                (select 'SERVICE' as type, vs.static_service_id as id, version, vs.name, vs.description, price as oldPrice, (price * (1 - sale_percentage)) as price,
                                        is_available, is_active, images, ss.service_category_id as category, vs.is_last_version,
                                        coalesce((select avg(lr.grade) from listing_review lr where lr.service_static_service_id = ss.static_service_id and lr.pending_status = 'APPROVED'), 0.0) as rating
                                from versioned_service vs
                                inner join static_service ss on ss.static_service_id = vs.static_service_id
                                inner join versioned_service_images vsi on vsi.versioned_service_static_service_id = vs.static_service_id and vsi.versioned_service_version = vs.version
                                ...
                                (select 'PRODUCT' as type, vp.static_product_id as id, version, vp.name, vp.description, price as oldPrice, (price * (1 - sale_percentage)) as price,
                                        is_available, is_active, images, sp.product_category_id as category, vp.is_last_version,
                                        coalesce((select avg(lr.grade) from listing_review lr where lr.product_static_product_id = sp.static_product_id and lr.pending_status = 'APPROVED'), 0.0) as rating
                                from versioned_product vp
                                inner join static_product sp on sp.static_product_id = vp.static_product_id
                                inner join versioned_product_images vpi on vpi.versioned_product_static_product_id = vp.static_product_id and vpi.versioned_product_version = vp.version
